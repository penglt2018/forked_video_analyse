...
def match_lkj(lkj_file, video_info, yolo_result):
    ...
    if lkj_rd_rt[0] == False:
        ...
        if no_break != [] and no_break != [[]]:
            try:
                no_break_final = LKJLIB.time_filter(no_break,lkj_df,speed_thresh=1, time_range=5)
        ...
        if cellphone != [] and cellphone != [[]]:
            try:
                cellphone_final = LKJLIB.time_filter(cellphone,lkj_df,speed_thresh=0, time_range=2)
...
def store_result(final_result, video_info, mysql_db, video_obj):
    ...
    global yolo_logger
    # resolve 
    no_break_final, sleep_final, cellphone_final, stand_2_final = final_result
    video_name = video_info[3]
    store_flag = True
    ...
    if no_break_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, no_break_final, 'violate_result.report', '未握大闸', video_obj, 'store/no_break') == False:
    ...
    if cellphone_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, cellphone_final, 'violate_result.report', '手机', video_obj, 'store/cellphone') == False:
    ...
    if stand_2_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, stand_2_final, 'violate_result.report', '副司机未立岗') == False:
