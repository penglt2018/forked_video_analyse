...
def exe_openpose(frame_mat, openpose_result, video_info, fps, video_width, video_height):
    ''' Function for executing openpose detect function and
        ...
        return:
                rt_flag: True / False
    '''
    global openpose_logger, box_info, openpose
    openpose_logger.info('Prepare for running openpose')
    rt_flag = True
    video_st_time = video_info[1]
    video_name = video_info[3]
    train_type = video_info[4]
    train_num = video_info[5]
    channel = video_info[6]
    ...
    if main_drive_box_data == []:
        ...
        openpose_result.info('Executing openpose to video {0}'.format(video_name))
        ...
        for pix_mat, frame_idx in frame_mat:
            try:
                ...
                main_driver_rt = coord_handler.get_driver_coord(pos_keypoint_arr, main_drive_box_data)
                pos_flag = True
            except Exception:
            ...
            if pos_flag == True:
                main_driver_pos = main_driver_rt[1:-1]
                ...
                if main_driver_pos[4] == -1:    # no person detected
                    ...
                    if wrong_head_rt != 0: # model output wrong head
                        common.add_result(openpose_result, frame_idx, fps, video_st_time, '主司机偏头')
                    ...
                    try:
                        ...
                        rt_flag = False
                    if nap_rt != 0:
                        common.add_result(openpose_result, frame_idx, fps, video_st_time, '主司机盹睡')
                    ...
                    if arm_detect_rt != 1 : # no person and normal case
                        if arm_detect_rt == 2 or arm_detect_rt == 0:
                            common.add_result(openpose_result, frame_idx, fps, video_st_time, '手比')
                        elif arm_detect_rt == 3 or arm_detect_rt == 0:
                            common.add_result(openpose_result, frame_idx, fps, video_st_time, '握拳')
                        elif arm_detect_rt == 4:
                            common.add_result(openpose_result, frame_idx, fps, video_st_time, '非标准手比')
                        else:
            ...
            try:
                ...
                for cus_pos in cus_pos_keypoints_list:
                    tf_model_result = sess.run(y_conv, feed_dict={x: np.array([cus_pos[i]], dtype=np.float32), keep_prob:1.0}).tolist()[0]
                    prob = max(tf_model_result)
                    max_index = tf_model_result.index(prob)
                    # label 1 = sleep, 0 = normal & sit, 2 = stand
                    if max_index == 1 and prob > 0.5:
                        common.add_result(openpose_result, frame_idx, fps, video_st_time, '平躺睡觉')
            except Exception:
...
def match_lkj(lkj_file, video_info, op_result):
    ...
    if lkj_rd_rt[0] == False:
        ...
        leave_result = [ i for i in op_result if i[-1] == '离岗' ]
        openpose_logger.info('Leave list build successfully')
        wrong_head_result = [ i for i in op_result if i[-1] == '偏头' ]
        ...
        point_forward_result = [ i for i in op_result if i[-1] == '手比' ]
        openpose_logger.info('Point list build successfully')
        fist_result = [ i for i in op_result if i[-1] == '握拳' ]
        openpose_logger.info('Fist list build successfully')
        other_gesture_result = [ i for i in op_result if i[-1] == '非标准手比' ]
        ...
        if leave_result != []:
            try:
                leave_final = LKJLIB.time_filter(leave_result, lkj_df, speed_thresh=1, time_range=3))
        ...
        if wrong_head_result != []:
            try:
                wrong_head_final = LKJLIB.time_filter(wrong_head_result, lkj_df, speed_thresh=1, time_range=3))
        ...
        if num_people_result != []:
            ...
            try:
                channel_shift_final = common.channel_shift(num_people_result, lkj_df, video_st_time, video_ed_time)
        ...
        # execute sleep rules
            if sleep_result != []:
                try:
                    sleep_final = LKJLIB.time_filter(sleep_result, lkj_df, speed_thresh=0, time_range=10))
    ...
    final_result = ([leave_final] + [wrong_head_final] + [single_person_final] + [channel_shift_final] + [sleep_final] + [point_forward_final] + [fist_final])
...
def store_result(final_result, video_info, mysql_db, video_obj, final_include_result=None):
    ...
    '''
    global openpose_logger
    leave_final, wrong_head_final, single_person_final, channel_shift_final, sleep_final, point_forward_final, fist_final = final_result
    ...
    if leave_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, leave_final, 'violate_result.report', '离岗', video_obj, 'store/leave') == False:
    ...
    if wrong_head_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, wrong_head_final, 'violate_result.report', '偏头', video_obj, 'store/wronghead') == False:
    ...
    if single_person_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, single_person_final, 'violate_result.report', '单人值乘', video_obj, 'store/single_driver') == False:
    ...
    if point_forward_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, point_forward_final, 'violate_result.report', '未手比', video_obj) == False:
    ...
    if fist_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, fist_final, 'violate_result.report', '未摇臂', video_obj) == False:
    ...
    if point_forward_include_final != []:
        common.add_to_db(mysql_db, video_info, point_forward_include_final, 'violate_result.report', '手比', video_obj)
    if fist_include_final != []:
        common.add_to_db(mysql_db, video_info, fist_include_final, 'violate_result.report', '摇臂', video_obj)

    other_final = point_forward_other_final + fist_other_final
    if other_final != []:
        common.add_to_db(mysql_db, video_info, other_final, 'violate_result.report', '不标准手比', video_obj)
