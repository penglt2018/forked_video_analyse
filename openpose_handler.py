...
def match_lkj(lkj_file, video_info, op_result):
    ...
    if lkj_rd_rt[0] == False:
        ...
        if nap_result != []:
            try:
                nap_final = LKJLIB.time_filter(nap_result, lkj_df, speed_thresh=0, time_range=5)
...
def store_result(final_result, video_info, mysql_db, video_obj, final_include_result=None):
    ...
    if wrong_head_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, wronghead_move, 'violate_result.report', '主司机偏头(运行)', video_obj, 'store/wronghead') == False or \
            common.add_to_db(mysql_db, video_info, wronghead_stop, 'violate_result.report', '主司机偏头(停车)', video_obj, 'store/wronghead') == False:
    ...
    if nap_final == []:
        ...
        if common.add_to_db(mysql_db, video_info, nap_move, 'violate_result.report', '主司机盹睡(运行)', video_obj, 'store/nap') == False or \
