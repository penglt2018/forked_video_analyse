...
def getConnect(host, port, username, password):
    ...
    try:
        ...
        result = [True, "ftp connection success", ftp]
    except Exception as e:
        result = [False, "ftp connection fail, reason:{0}".format(traceback.format_exc())]
    return result
...
#         result[1] = result[1] + "\n" + rs[1]
#     except Exception as e:
#         result = [-1, "download fail, reason:{0}".format(traceback.format_exc())]
...
def downloadFile(ftp, remoteRelPath, localDir):
    """
    :param ftp:
    ...
    try:
        ...
        splitPaths = os.path.split(localPath)
        ...
        handle = open(localPath, "wb")
        ftp.retrbinary("RETR %s" % remoteRelPath, handle.write, 1024)
        handle.close()

        result = [1, "ftp download " + splitPaths[1] + " success"]
    except Exception as e:
        result = [-1, "ftp download fail, reason:{0}".format(traceback.format_exc())]
    return result
...
#     except Exception as e:
#         result = [-1, "upload fail, reason:{0}".format(traceback.format_exc())]
